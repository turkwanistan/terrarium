...
Current remaining release gates are now narrower: install the validated PCK + matching generated HTML metadata into `display/web/godot/`, then run ordinary-browser HTTPS UAT against the living world. Local promotion is presently blocked only by stale filesystem ownership on that generated directory (`nobody:nogroup`); the validated production-safe replacement files are staged under `artifacts/godot-art-gate/web-deep-debug/validated-production/`. No permissions were weakened and no fixture-only browser shim was promoted. Iteration 10 remains blocked until the ordinary living-world browser UAT closes the migration gate or the migration is deliberately deferred.
